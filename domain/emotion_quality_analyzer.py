...
class EmotionQualityAnalyzer:
    ...
    async def analyze_conversation(
        ...
        try:
            ...
            recommendations = await self._generate_recommendations(
                eqs_score, metrics, warning_flags
            )
    ...
    async def _calculate_eqs_score(
        self, metrics: EmotionalMetrics, messages: List[Dict[str, Any]]
    ) -> float:
        """EQS (Emotional Quality Score) hesapla"""

        # Ağırlıklı ortalama
        eqs = (
            metrics.text_depth * 0.20
            + metrics.response_quality * 0.25  # %20 metin derinliği
            + metrics.emotional_consistency * 0.15  # %25 yanıt kalitesi
            + metrics.intimacy_progression * 0.20  # %15 tutarlılık
            + metrics.satisfaction_potential  # %20 yakınlık ilerlemesi
            * 0.20  # %20 tatmin potansiyeli
        )
    ...
    def _determine_intimacy_level(
        self, eqs_score: float, metrics: EmotionalMetrics
    ) -> IntimacyLevel:
        """Yakınlık seviyesi belirle"""
        if eqs_score >= 0.8:
    ...
    async def _detect_emotional_state(
        self, messages: List[Dict[str, Any]], metrics: EmotionalMetrics
    ) -> EmotionalState:
        """Duygusal durum tespit et"""
    ...
    async def _generate_recommendations(
        self, eqs_score: float, metrics: EmotionalMetrics, warnings: List[str]
    ) -> List[str]:
        """Öneriler oluştur"""
        recommendations = []
    ...
    def _create_fallback_analysis(
        self, user_id: str, character_id: str, msg_count: int
    ) -> ConversationAnalysis:
