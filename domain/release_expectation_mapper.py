...
class ReleaseExpectationMapper:
    ...
    async def track_satisfaction(
        ...
        try:
            ...
            next_cycle_prediction = await self._predict_next_cycle(
                user_id, satisfaction_analysis["score"]
            )
            ...
            event = SatisfactionEvent(
                ...
                user_id=user_id,
                character_id=character_id,
                satisfaction_level=self._determine_satisfaction_level(
                    satisfaction_analysis["score"]
                ),
    ...
    async def _determine_cycle_phase(
        self, user_id: str, expectation_analysis: Dict[str, Any]
    ) -> CyclePhase:
    ...
    async def _complete_cycle(self, satisfaction_event: SatisfactionEvent) -> None:
        ...
        cycle.satisfaction_ratio = satisfaction_event.satisfaction_score / max(
            total_expectation, 0.1
        )
    ...
    async def _calculate_efficiency(self, cycle: DopamineCycle) -> float:
        ...
        efficiency = satisfaction_efficiency * 0.5 + time_efficiency * 0.3 + moment_efficiency * 0.2
    ...
    async def _determine_release_type(
        self, content: str, last_expectation: Optional[ExpectationMoment]
    ) -> ExpectationType:
        ...
        if any(
            re.search(p, content_lower) for p in self.expectation_patterns["physical_indicators"]
        ):
            return ExpectationType.PHYSICAL_RELEASE
        elif any(
            re.search(p, content_lower) for p in self.expectation_patterns["emotional_buildup"]
        ):
    ...
    async def _predict_next_cycle(self, user_id: str, satisfaction_score: float) -> float:
        ...
        for i in range(1, len(user_cycles)):
            interval = (
                user_cycles[i].start_time - user_cycles[i - 1].end_time
            ).total_seconds() / 3600
    ...
    async def _update_user_profile_satisfaction(self, event: SatisfactionEvent) -> None:
        ...
        if user_id in self.user_profiles:
            ...
            user_cycles = [c for c in self.completed_cycles if c.user_id == user_id]
            if user_cycles:
                avg_addictiveness = sum(c.addictiveness_score for c in user_cycles) / len(
                    user_cycles
                )
    ...
    async def _update_satisfaction_stats(self, event: SatisfactionEvent) -> None:
        ...
        if self.daily_stats["total_expectations"] > 0:
            self.daily_stats["satisfaction_rate"] = (
                self.daily_stats["total_satisfactions"] / self.daily_stats["total_expectations"]
            )
    ...
    async def generate_expectation_report(self, user_id: str) -> Dict[str, Any]:
        ...
        satisfaction_rate = (
            total_satisfactions / total_expectations if total_expectations > 0 else 0
        )

        # Average scores
        avg_expectation_intensity = sum(e.intensity for e in user_expectations) / total_expectations
        avg_satisfaction_score = (
            sum(s.satisfaction_score for s in user_satisfactions) / total_satisfactions
            if total_satisfactions > 0
            else 0
        )
        ...
        if user_cycles:
            cycle_stats = {
                ...
                "average_efficiency": sum(c.efficiency_score for c in user_cycles)
                / len(user_cycles),
                "average_addictiveness": sum(c.addictiveness_score for c in user_cycles)
                / len(user_cycles),
