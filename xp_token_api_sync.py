#!/usr/bin/env python3
"""
🪙 XP Token API v1.0 (Sync Version) 🪙

OnlyVips v6.0 XP/Token sistemi için özel API
Flask synchronous version - async compatibility issues fixed
...
def get_user_stats(user_id):
    ...
    try:
        ...
        user_stats = run_async_function(get_stats())
...
def spend_user_tokens(user_id):
    ...
    try:
        ...
        async def spend_tokens():
            ...
            if not can_afford:
                return False, f"Insufficient balance. Need {cost} tokens, have {balance}", None, None, None
            ...
            if success:
                ...
                return True, message, cost, balance, new_balance
            else:
                return False, message, None, None, None
        
        success, message, cost, balance, new_balance = run_async_function(spend_tokens())
...
def award_user_xp(user_id):
    ...
    try:
        ...
        async def award_xp():
            from xp_token_engine.bot_integration import award_user_xp as award_xp_func
            return await award_xp_func(int(user_id), action, bonus_multiplier)
        
        success, tokens, message = run_async_function(award_xp())
...
def get_system_status():
    ...
    try:
        async def get_status():
            from xp_token_engine.token_manager import get_token_manager
            async with get_token_manager() as tm:
                return await tm.get_all_users_stats()
        
        system_stats = run_async_function(get_status())
        ...
        status_data = {
            ...
            "total_tokens": system_stats.get('total_tokens', 0),
            "total_transactions": system_stats.get('total_transactions', 0),
...
def get_recent_logs():
    ...
    try:
        ...
        recent_logs = run_async_function(get_logs())
...
def get_leaderboard():
    ...
    try:
        ...
        system_stats = run_async_function(get_leaderboard_data())
...
def api_documentation():
    """API documentation"""
    docs = {
        "service": "XP Token API v1.0 (Sync)",
        "description": "OnlyVips v6.0 XP/Token Economy API - Synchronous Version",
        "endpoints": {
...
if __name__ == "__main__":
    logger.info("🪙 XP Token API v1.0 (Sync) starting...")
