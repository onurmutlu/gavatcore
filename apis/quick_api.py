...
@app.get("/")
async def root():
    return {"message": "GavatCore Character API", "version": "1.0.0", "status": "running"}

@app.get("/characters")
async def get_characters():
    """Get all characters"""
    print(f"🔄 API: Getting all characters - Found {len(characters_data)} characters")
    return {
        "success": True,
...
async def get_character(character_id: str):
    ...
    print(f"🔄 API: Getting character {character_id}")
...
async def update_character(character_id: str, update: CharacterUpdate):
    """Update a character"""
    print(f"🔄 API: Updating character {character_id} with data: {update.dict()}")
    if character_id not in characters_data:
        raise HTTPException(status_code=404, detail="Character not found")
    ...
    print(f"✅ API: Character {character_id} updated successfully")
    return {
        "success": True,
...
async def test_character_reply(character_id: str, data: dict):
    ...
    print(f"🔄 API: Testing reply for character {character_id}")
    if character_id not in characters_data:
        raise HTTPException(status_code=404, detail="Character not found")
    ...
    print(f"✅ API: Generated reply for {character_id}: {reply}")
    return {
        "success": True,
        "reply": reply,
...
if __name__ == "__main__":
    ...
    print("🔄 CORS enabled for Flutter web access")
